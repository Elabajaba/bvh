    ...
    fn get_children_node_data(&self) -> Option<(NodeData, NodeData)> {
        match *self {
            BVHNode::Node {
                ...
                NodeData {
                    index: child_l_index,
                    aabb: child_l_aabb.unwrap(),
                },
                NodeData {
                    index: child_r_index,
                    aabb: child_r_aabb.unwrap(),
                },
            )),
            BVHNode::Leaf { .. } => None,
        }
...
impl BVH {
        ...
        match self.nodes[node_index] {
            ...
            BVHNode::Node {
                ...
                ) = (&self.nodes[child_l_index], &self.nodes[child_r_index])
                {
                    ...
                    {
                        *child_l_aabb = Some(shapes[shape_l_index].aabb());
                        *child_r_aabb = Some(shapes[shape_r_index].aabb());
    ...
    fn try_rotate<Shape: BHShape>(
        ...
        if let Some((rotation_node_a, rotation_node_b)) = best_rotation {
            ...
            *self.nodes[node_index].child_l_aabb_mut() = Some(child_l_aabb);
            *self.nodes[node_index].child_r_aabb_mut() = Some(child_r_aabb);
    ...
    fn fix_children_and_own_aabbs<Shape: BHShape>(&mut self, node_index: usize, shapes: &[Shape]) {
        ...
        *self.nodes[node_index].child_l_aabb_mut() =
            Some(self.nodes[child_l_index].get_node_aabb(shapes));
        *self.nodes[node_index].child_r_aabb_mut() =
        Some(self.nodes[child_r_index].get_node_aabb(shapes));
    }
            ...
            match self.nodes[parent_index] {
                BVHNode::Node {
                    ...
                    if left_child {
                        ...
                        *child_l_aabb = Some(child_aabb);
                    } else {
                        *child_r_index = child_index;
                        *child_r_aabb = Some(child_aabb);
                    }
...
mod tests {
    ...
    fn test_consistent_after_optimize() {
        ...
        shapes[0].pos = Vec3::new(10.0, 1.0, 2.0);
        shapes[1].pos = Vec3::new(-10.0, -10.0, 10.0);
        shapes[2].pos = Vec3::new(-10.0, 10.0, 10.0);
        shapes[3].pos = Vec3::new(-10.0, 10.0, -10.0);
        shapes[4].pos = Vec3::new(11.0, 1.0, 2.0);
        shapes[5].pos = Vec3::new(11.0, 2.0, 2.0);
    ...
    fn test_optimize_simple_update() {
        ...
        shapes.push(UnitBox::new(0, Vec3::new(-50.0, 0.0, 0.0)));
        shapes.push(UnitBox::new(1, Vec3::new(-40.0, 0.0, 0.0)));
        shapes.push(UnitBox::new(2, Vec3::new(50.0, 0.0, 0.0)));
        ...
        shapes[1].pos = Vec3::new(40.0, 0.0, 0.0);
    ...
    fn create_predictable_bvh() -> (Vec<UnitBox>, BVH) {
        let mut shapes = Vec::new();
        shapes.push(UnitBox::new(0, Vec3::new(0.0, 0.0, 0.0)));
        shapes.push(UnitBox::new(1, Vec3::new(2.0, 0.0, 0.0)));
        shapes.push(UnitBox::new(2, Vec3::new(4.0, 0.0, 0.0)));
        shapes.push(UnitBox::new(3, Vec3::new(6.0, 0.0, 0.0)));
        ...
        nodes.push(BVHNode::Node {
            parent_index: 0,
            depth: 0,
            child_l_aabb: Some(shapes[0].aabb().join(&shapes[1].aabb())),
            child_l_index: 1,
            child_r_aabb: Some(shapes[2].aabb().join(&shapes[3].aabb())),
            child_r_index: 2,
        });
        ...
        nodes.push(BVHNode::Node {
            parent_index: 0,
            depth: 1,
            child_l_aabb: Some(shapes[0].aabb()),
            child_l_index: 3,
            child_r_aabb: Some(shapes[1].aabb()),
            child_r_index: 4,
        });
        nodes.push(BVHNode::Node {
            parent_index: 0,
            depth: 1,
            child_l_aabb: Some(shapes[2].aabb()),
            child_l_index: 5,
            child_r_aabb: Some(shapes[3].aabb()),
            child_r_index: 6,
        });
    ...
    fn test_connect_grandchildren() {
        ...
        assert!(nodes[1]
            .child_l_aabb()
            .relative_eq(&shapes[2].aabb()));
        assert!(nodes[1]
            .child_r_aabb()
            .relative_eq(&shapes[1].aabb()));
        assert!(nodes[2]
            .child_l_aabb()
            .relative_eq(&shapes[0].aabb()));
        assert!(nodes[2]
            .child_r_aabb()
            .relative_eq(&shapes[3].aabb()));
    }
    ...
    fn test_try_rotate_child_grandchild() {
        ...
        shapes[2].pos = Vec3::new(-40.0, 0.0, 0.0);
        ...
        assert!(nodes[0]
            .child_r_aabb()
            .relative_eq(&right_subtree_aabb));
